...
@foreach ($usersRandom as $user)
    <div class="w-full hover:bg-gray-800 hover:text-gray-200">
        <div class="flex flex-col w-full lg:w-[270px] gap-2 pt-2 pl-2 text-left md:flex-col lg:flex-row md:items-center">

            <img src="{{ $user->photo }}" class="w-12 h-12 bg-cover rounded-full" alt="avatar" />
            <a class="p-0 m-1" href="/profil/{{ $user->id }}">
                <div class="font-bold text-gray-400 hover:text-white break-all">{{ $user->name }}</div>
            </a>
            <div class="flex lg:max-w-[270px] md:justify-center lg:justify-end">
                <button
